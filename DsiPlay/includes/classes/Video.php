...
class Video {
    ...
    public function incrementViews() {
        $query = $this->con->prepare("UPDATE video SET views = views + 1 WHERE id=:id");
        $videoId= $this->getId();
    ...
    public function like(){
        ...
        $id=$this->getId();
        $username= $this->userLoggedInObj->getUn();
        if($this->wasLikedBy()){
    ...
    public function getNoOfComment() {
        $query= $this->con->prepare("SELECT * FROM commments WHERE videoId=:videoId");
        $query->bindParam(":videoId",$id);
        $id = $this->getId();
